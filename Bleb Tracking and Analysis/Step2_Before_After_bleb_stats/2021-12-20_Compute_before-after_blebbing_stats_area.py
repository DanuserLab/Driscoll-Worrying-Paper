...
if __name__=="__main__":
    ...
    savepicformat = '.svg'
    jitter = 0.05
    aspect = (2,5) # what we have at the moment. 
...
# =============================================================================
    ...
    for ii in np.arange(len(all_datafolders))[:]:
        ...
        """
        Compile the before and after for the series. 
        """
        R = []
        loc = []
        arp = []
        ...
        for jj in np.arange(len(resp_cells_R)):
            ...
            cutoff_frame = len(signal_ii) // 2
            before_select = cell_size[:,0]<= cutoff_frame
            after_select = cell_size[:,0] > cutoff_frame
            # # zscore. 
            # pop_mean = np.nanmean(signal_ii)
            # pop_std = np.nanstd(signal_ii)
            # zscore_size = (cell_size[:,1] - pop_mean) / pop_std
            before = np.nanmean(cell_size[cell_size[:,0]<= cutoff_frame, 1]**2*np.pi) # using pi * r * 2
            after = np.nanmean(cell_size[cell_size[:,0] > cutoff_frame, 1]**2*np.pi)
            # before = np.nanmean(zscore_size[before_select])
            # after = np.nanmean(zscore_size[after_select])
            
            R.append([before, after])
            loc.append([np.nanmean(localization_ii[:cutoff_frame+1]), np.nanmean(localization_ii[cutoff_frame+1:])])
            arp.append([np.nanmean(arp_ii[:cutoff_frame+1]), np.nanmean(arp_ii[cutoff_frame+1:])])
        ...
        if '-' not in analyse_movies:
            ...
            ######
            # convert the use_regions into indexes to iterate over. 
            
            # iterate over the use_regions. 
            
            all_resp_cells_R.append(R[use_regions])
            all_resp_cells_localization.append(loc[use_regions])
            all_resp_cells_arp.append(arp[use_regions])
    ...
    all_resp_cells_R = np.vstack(all_resp_cells_R)
    all_resp_blebs_mean = np.nanmean(all_resp_cells_R, axis=0)
    all_resp_blebs_sem = spstats.sem(all_resp_cells_R, axis=0)
    
    all_resp_cells_localization = np.vstack(all_resp_cells_localization)
    all_resp_blebs_loc_mean = np.nanmean(all_resp_cells_localization, axis=0)
    all_resp_blebs_loc_sem = spstats.sem(all_resp_cells_localization, axis=0)

    all_resp_cells_arp = np.vstack(all_resp_cells_arp)
    arp_select = ~np.isnan(all_resp_cells_arp[:,0])
    all_resp_cells_arp = all_resp_cells_arp[arp_select]
    
    all_resp_blebs_arp_mean = np.nanmean(all_resp_cells_arp, axis=0)
    all_resp_blebs_arp_sem = spstats.sem(all_resp_cells_arp, axis=0)
    
    all_resp_cells_movies = np.hstack(all_resp_cells_movies)
    
    
# =============================================================================
#     Save all the collected together statistics so we can do co-analysis. 
# =============================================================================

    savematfile = '2021-12-21_before-after-bleb-stats-responsive_for_coanalysis-Area.mat'
...
# =============================================================================
#     Do the statistical tests and plots. 
# =============================================================================
    ...
    stat test (ks test)
    """
    kstest, ks_pval = spstats.ks_2samp(all_resp_cells_R[:,0], all_resp_cells_R[:,1], alternative='greater')
    print(ks_pval)
    
    ttest, ttest_pval = spstats.ttest_rel(all_resp_cells_R[:,0], all_resp_cells_R[:,1])#, alternative='greater')
    print('difference in bleb size before/after', ttest_pval)
    
    ttest, ttest_pval = spstats.ttest_rel(all_resp_cells_localization[:,0], all_resp_cells_localization[:,1])#, alternative='greater')
    print('difference in loc before/after', ttest_pval)
    
    ttest, ttest_pval = spstats.ttest_rel(all_resp_cells_arp[:,0], all_resp_cells_arp[:,1])#, alternative='greater')
    print('difference in arp3 before/after', ttest_pval)
    ...
    #        resp_blebs_mean)
    ax.errorbar(np.arange(all_resp_cells_R.shape[1]), 
                all_resp_blebs_mean, 
                yerr = all_resp_blebs_sem, 
                fmt='none', 
                capsize=5, capthick=2, lw=2, ecolor='k')
    for group_ii in np.arange(len(all_resp_blebs_mean)):
        data = all_resp_cells_R[:,group_ii]
        ...
        ax.plot([group_ii-0.1, group_ii+0.1],
                [all_resp_blebs_mean[group_ii], all_resp_blebs_mean[group_ii]], 
                'k-', lw=2, zorder=1000)
    # plt.ylim([0.4,1.8])
    plt.ylim([0, 12])
    ...
    plt.xticks(np.arange(len(all_resp_blebs_mean)), ['Before', 'After'], fontsize=14, fontname='Arial')
    plt.yticks(fontsize=14, fontname='Arial')
    plt.ylabel('Bleb Area ($\mu$m$^2$)', fontname='Arial', fontsize=16)
    # take the spines out. 
    # save the analysis out. 
    plt.savefig(os.path.join(savefolder, 'Before-After-bleb-size_responsive-Area_%s' %(savepicformat)), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # ax.bar(np.arange(resp_blebs.shape[1]), 
    #        resp_blebs_mean)
    ax.errorbar(np.arange(all_resp_cells_localization.shape[1]), 
                all_resp_blebs_loc_mean, 
                yerr = all_resp_blebs_loc_sem, 
                fmt='none', 
                capsize=5, capthick=2, lw=2, ecolor='k')
    for group_ii in np.arange(len(all_resp_blebs_loc_mean)):
        data = all_resp_cells_localization[:,group_ii]
        data_xx = group_ii + jitter*np.random.randn(len(data))
        ax.plot(data_xx, data, 'o', mfc='none', zorder=100)
        ax.plot([group_ii-0.1, group_ii+0.1],
                [all_resp_blebs_loc_mean[group_ii], all_resp_blebs_loc_mean[group_ii]], 
                'k-', lw=2)
    # plt.ylim([0.4,1.8])
    plt.ylim([0.,3.5])
    plt.xlim([-1+0.5,2-0.5])
    ...
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    plt.xticks(np.arange(len(all_resp_blebs_loc_mean)), ['Before', 'After'], fontsize=14, fontname='Arial')
    plt.yticks(fontsize=14, fontname='Arial')
    plt.ylabel('Localisation (a.u.)', fontname='Arial', fontsize=16)
    # take the spines out. 
    # save the analysis out. 
    plt.savefig(os.path.join(savefolder, 'Before-After_responsive_localisation%s' %(savepicformat)), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # ax.bar(np.arange(resp_blebs.shape[1]), 
    #        resp_blebs_mean)
    ax.errorbar(np.arange(all_resp_cells_arp.shape[1]), 
                all_resp_blebs_arp_mean, 
                yerr = all_resp_blebs_arp_sem, 
                fmt='none', 
                capsize=5, capthick=2, lw=2, ecolor='k')
    for group_ii in np.arange(len(all_resp_blebs_arp_mean)):
        data = all_resp_cells_arp[:,group_ii]
        data_xx = group_ii + jitter*np.random.randn(len(data))
        ax.plot(data_xx, data, 'o', mfc='none', zorder=100)
        ax.plot([group_ii-0.1, group_ii+0.1],
                [all_resp_blebs_arp_mean[group_ii], all_resp_blebs_arp_mean[group_ii]], 
                'k-', lw=2)
    # plt.ylim([0.4,1.8])
    plt.ylim([0.,3.5])
    plt.xlim([-1+0.5,2-0.5])
    ...
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    plt.xticks(np.arange(len(all_resp_blebs_arp_mean)), ['Before', 'After'], fontsize=14, fontname='Arial')
    plt.yticks(fontsize=14, fontname='Arial')
    plt.ylabel('Arp3 (a.u.)', fontname='Arial', fontsize=16)
    # take the spines out. 
    # save the analysis out. 
    plt.savefig(os.path.join(savefolder, 'Before-After_responsive_arp3%s' %(savepicformat)), dpi=300, bbox_inches='tight')
    plt.show()
...
# # =============================================================================
    ...
    # savematfile = os.path.join(savefolder, 'xcorr_stats_nonresponsive_vs_arp3.mat')
    # spio.savemat(savematfile, 
